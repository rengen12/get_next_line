...
void	addfile(int fd, char *line, t_nxtl **files)
{
	...
	if(*files)
	{
        ...
        {
            ft_lstadd((void **)files, malloc(sizeof(t_nxtl)));
            //if (!(tmp->next = (t_nxtl *)malloc(sizeof(t_nxtl))))
                //return ;
            //tmp = tmp->next;
            //tmp->next = NULL;
        }
        //(tmp->fd != fd) ?  tmp = tmp->next : (void)fd;
	}
	else
	{
...
int		get_next_line(const int fd, char **line)
{
	...
	if (i)
    {
        ft_memmove(line[0], file->rem, i);
        //ft_memccpy(line[0], file->rem, '\n', i);
        //i = ft_strlen(line[0]);
        //filedel(&files, file->fd); not needed
    }
